...
| BROWSE_CN_INTERMEDIATE_VALUES_ENABLED       | false                     | Specifies if globally intermediate values (nested instance items) must be populated or not (tenant can override this setting)                                                         |
...
| `items.effectiveLocationId`         |   term    | `items.effectiveLocationId=="1212"`                          | Matches instances that have item with the effective location                                                        |
| `items.statisticalCodeIds`          |   term    | `items.statisticalCodeIds=="1212"`                           | Matches instances that have item with the statistical code Id                                                       |
| `items.status.name`                 |   term    | `items.status.name=="Available"`                             | Matches instances that have item with given status                                                                  |
| `items.materialTypeId`              |   term    | `items.materialTypeId="23434"`                               | Matches instances that have item with given material type                                                           |
| `items.discoverySuppress`           |   term    | `items.discoverySuppress=true`                               | Matches instances that have item suppressed/not suppressed from discovery                                           |
...
| `items.effectiveShelvingOrder`      |   term    | `items.effectiveShelvingOrder="A 12"`                        | Matches instances that have item with given `effectiveShelvingOrder` value                                          |
...
| `items.metadata.createdDate`        |   term    | `items.metadata.createdDate > "2020-12-12"`                  | Matches instances with item that were created after  `2020-12-12`                                                   |
| `items.metadata.updatedDate`        |   term    | `items.metadata.updatedDate > "2020-12-12"`                  | Matches instances with item that were updated after  `2020-12-12`                                                   |
