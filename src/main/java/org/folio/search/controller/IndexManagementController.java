...
public class IndexManagementController implements IndexManagementApi {
  ...
  @Override
  public ResponseEntity<ReindexJob> reindexClassificationsRecords(String tenantId, ReindexClassificationsRequest request) {
    log.info("Attempting to start reindex for inventory [tenant: {}]", tenantId);
    return ResponseEntity.ok(indexService.reindexClassificationsRecords(tenantId, request));
  }
