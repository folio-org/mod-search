...
public class InstanceClassificationRepository extends AbstractResourceRepository {
  ...
  public FolioIndexOperationResponse indexResources(List<SearchDocumentBody> documentBodies) {
    ...
    for (var entry : docsById.entrySet()) {
      ...
      if (!IterableUtils.matchesAll(instances.values(), Set::isEmpty)) {
        ...
        if (doc.getAction() == DELETE) {
          for (SearchDocumentBody document : documents) {
            if (document.getAction() == INDEX) {
              doc = document;
              break;
            }
          }
        }
        var upsertRequest = prepareUpsertRequest(doc, instances);
  ...
  private EnumMap<IndexActionType, Set<Map<String, Object>>> prepareInstances(List<SearchDocumentBody> documents) {
    ...
    for (var document : documents) {
      var payload = getInitialPayload(document);
  ...
  private UpdateRequest prepareUpsertRequest(SearchDocumentBody doc,
    ...
    return new UpdateRequest()
      ...
      .script(new Script(STORED, null, INSTANCE_CLASSIFICATION_UPSERT_SCRIPT_ID, prepareScriptParams(instances)))
  ...
  private Map<String, Object> getPayload(SearchDocumentBody doc) {
    if (doc.getAction() == DELETE) {
      return getInitialPayload(doc);
    } else {
      return jsonConverter.fromJson(doc.getDocumentBody().utf8ToString(), MAP_TYPE_REFERENCE);
    }
  }

  private Map<String, Object> getInitialPayload(SearchDocumentBody doc) {
