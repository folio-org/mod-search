CREATE OR REPLACE FUNCTION dedup_staging_holding()
RETURNS TABLE (
...
BEGIN
    ...
    WITH staging_with_stats AS (
        SELECT
            id, tenant_id, instance_id, json, inserted_at,
            COUNT(*) OVER () AS total_records,
            ROW_NUMBER() OVER (PARTITION BY id, tenant_id ORDER BY inserted_at DESC) as rn
        FROM staging_holding
    ),
    deduped AS (
        SELECT
            id, tenant_id, instance_id, json,
            MAX(total_records) AS total_records
        FROM staging_with_stats
        WHERE rn = 1
        GROUP BY id, tenant_id, instance_id, json
    ),
    inserted AS (
        INSERT INTO holding (id, tenant_id, instance_id, json)
        SELECT id, tenant_id, instance_id, json FROM deduped
