...
class SearchLinkedDataInstanceIT extends BaseIntegrationTest {
  ...
  void searchByLinkedDataInstance_parameterized_singleResult(int index, String query) throws Throwable {
    doSearchByLinkedDataInstance(query)
      ...
      .andExpect(jsonPath(toIdValue(toRootContent(), 2), is("a b24123456 ")))
      .andExpect(jsonPath(toIdType(toRootContent(), 2), is("LCCN")))
      .andExpect(jsonPath(toIdValue(toRootContent(), 3), is("0987654321")))
      .andExpect(jsonPath(toIdType(toRootContent(), 3), is("IAN")))
