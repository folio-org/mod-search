...
public class ParsedStringStats extends ParsedAggregation {
  ...
  public static final ConstructingObjectParser<ParsedStringStats, String> PARSER = new ConstructingObjectParser<>("string_stats", true, (args, name) -> {
    long count = (Long)args[0];
    boolean disributionWasExplicitNull = args[5] == NULL_DISTRIBUTION_MARKER;
    if (count == 0L) {
      return new ParsedStringStats(name, count, 0, 0, 0.0, 0.0, disributionWasExplicitNull, (Map)null);
    } else {
      ...
      if (disributionWasExplicitNull) {
        return new ParsedStringStats(name, count, minLength, maxLength, averageLength, entropy, disributionWasExplicitNull, (Map)null);
      } else {
        Map<String, Double> distribution = (Map)args[5];
        return new ParsedStringStats(name, count, minLength, maxLength, averageLength, entropy, distribution != null, distribution);
      }
    }
  });

  private ParsedStringStats(String name, long count, int minLength, int maxLength, double avgLength, double entropy, boolean showDistribution, Map<String, Double> distribution) {
    this.setName(name);
    this.count = count;
  ...
  static {
    ...
    PARSER.declareObjectOrNull(ConstructingObjectParser.optionalConstructorArg(), (p, c) -> {
      return Collections.unmodifiableMap(p.map(HashMap::new, XContentParser::doubleValue));
